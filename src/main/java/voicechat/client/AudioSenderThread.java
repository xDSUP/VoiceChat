...
public class AudioSenderThread extends Thread{
    ...
    public AudioSenderThread(ClientDesktop app, ObjectOutputStream outputStream) throws LineUnavailableException {
        super();
        this.app = app;
    ...
    @Override
    public void run(){
        ClientDesktop.LOGGER.info("CAPTURE AUDIO THREAD START");
        try{
            while(true){
                if(!app.microOn){
                    Utils.sleep(10);
        ...
        ClientDesktop.LOGGER.info("CAPTURE AUDIO = OFF");
    }
}